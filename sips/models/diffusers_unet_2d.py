...
class UNet2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                if attention_head_dim is not None else output_channel,
        ...
        self.mid_block = UNetMidBlock2D(
            ...
            resnet_time_scale_shift=resnet_time_scale_shift,
            attention_head_dim=attention_head_dim
            if attention_head_dim is not None else block_out_channels[-1],
            resnet_groups=norm_num_groups,
    ...
    def forward(
        ...
        Args:
            ...
            sampling_args (`Any`, *optional*, defaults to `None`): The sampling pipeline for generating images.
        ...
        if sampling_args:
            return self.sample_model(sampling_args)
    ...
    def sample_model(self, sampling_args) -> Tuple:
        # Generate images.

        if 'trajectory' in sampling_args.keys():
            if sampling_args['trajectory']:

                images_shape = (
                    sampling_args['num_samples'],
                    self.in_channels,
                    *self.sample_size,
                    len(sampling_args['noise_scheduler']),
                )

                images = torch.randn(
                    [*images_shape],
                    device=sampling_args['device'],
                )

                image = images[..., 0]

                i = 0
                for t in tqdm(sampling_args['noise_scheduler'].timesteps):
                    # 1. predict noise model_output
                    model_output = self.forward(image, t).sample

                    # 2. compute previous image: x_t -> x_t-1
                    image = sampling_args['noise_scheduler'].step(
                        model_output,
                        t,
                        image,
                        generator=None,
            ...
            image = torch.randn(image_shape, device=sampling_args['device'])
